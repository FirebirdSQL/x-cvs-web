...
//		$trans_tbl = get_html_translation_table (HTML_SPECIALCHARS);
	...
	// -------------------------------------------------------------------
	// Replace special characters by real characters
	// -------------------------------------------------------------------
...
//			$cache_file = $this->cache_dir . '/rsscontent_' . md5($rss_url);
//			if ($f = @fopen($cache_file, 'w')) {
//				fwrite ($f, $rss_content);
//				fclose($f);
//			}
			...
			foreach($rss_items as $rss_item) {
				...
				if ($result['items'][$i]['description'])
					$result['items'][$i]['description'] = $this->unspecialchars(strip_tags($result['items'][$i]['description']));
				// Item counter
				$i++;
			}
			return $result;

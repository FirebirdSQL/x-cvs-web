...
if(!$result = mysql_query("SELECT * FROM catagories ORDER BY cat_id"))
    forumerror(0022);
...
do {
	$sub_sql = "SELECT f.*, u.uname FROM forums f, users u WHERE f.cat_id = '$row[cat_id]' AND f.forum_moderator = u.uid ORDER BY forum_id";
	...
	if($myrow = mysql_fetch_array($sub_result)) {
		...
		do {
			...
			if(($userdata["lastvisit"] < $last_post) && $last_post != "No posts") {
				echo "<TD BGCOLOR=\"$bgcolor2\" ALIGN=\"CENTER\" VALIGN=\"MIDDLE\" WIDTH=5%><IMG SRC=\"images/forum/icons/red_folder.gif\" Alt=\"\"></TD>";
			}
			else {
				echo "<TD BGCOLOR=\"$bgcolor2\" ALIGN=\"CENTER\" VALIGN=\"MIDDLE\" WIDTH=5%><IMG SRC=\"images/forum/icons/folder.gif\" Alt=\"\"></TD>";
			}
			$name = stripslashes($myrow[forum_name]);
			echo "<TD BGCOLOR=\"$bgcolor3\"><FONT SIZE=2 COLOR=\"$textcolor2\"><a href=\"viewforum.php?forum=$myrow[forum_id]\">$name </a></font>\n";
			...
			echo "<TD BGCOLOR=\"$bgcolor2\" WIDTH=5% ALIGN=\"CENTER\" VALIGN=\"MIDDLE\"><FONT SIZE=1 COLOR=\"$textcolor2\">".translate("Free for All")."</font></TD>\n";
			
			
			if ($myrow[forum_type] == "1")
			echo "<TD BGCOLOR=\"$bgcolor2\" WIDTH=5% ALIGN=\"CENTER\" VALIGN=\"MIDDLE\"><FONT SIZE=1 COLOR=\"$textcolor2\">".translate("Protected Area")."</font></TD>\n";
			
			
			
			if ($myrow[forum_access]=="1" && $myrow[forum_type] == "0")
			echo "<TD BGCOLOR=\"$bgcolor2\" WIDTH=5% ALIGN=\"CENTER\" VALIGN=\"MIDDLE\"><FONT SIZE=1 COLOR=\"$textcolor2\">".translate("Registered User")."</font></TD>\n";
			if ($myrow[forum_access]=="2" && $myrow[forum_type] == "0")
			echo "<TD BGCOLOR=\"$bgcolor2\" WIDTH=5% ALIGN=\"CENTER\" VALIGN=\"MIDDLE\"><FONT SIZE=1 COLOR=\"$textcolor2\">".translate("Moderator")."</font></TD>\n";
			// if ($myrow[forum_type] == "1")
			// echo "<TD BGCOLOR=\"$bgcolor2\" WIDTH=5% ALIGN=\"CENTER\" VALIGN=\"MIDDLE\"><FONT SIZE=1 COLOR=\"$textcolor2\">".translate("Protected Area")."</font></TD>\n";
			...
			echo "<TD BGCOLOR=\"$bgcolor2\" WIDTH=5% ALIGN=\"CENTER\" VALIGN=\"MIDDLE\"><FONT SIZE=1 COLOR=\"$textcolor2\">$total_posts</font></TD>\n";
			...
			echo "<TD BGCOLOR=\"$bgcolor2\" WIDTH=5% ALIGN=\"CENTER\" VALIGN=\"MIDDLE\"><FONT SIZE=1 COLOR=\"$textcolor2\"><a href=\"user.php?op=userinfo&uname=$forum_moderator\">$forum_moderator</a></font></TD></TR>\n";

...
if($submit) {
	...
	$time = date("Y-m-d H:i");
	$sql = "INSERT INTO forumtopics (topic_title, topic_poster, forum_id, topic_time, topic_notify) VALUES ('$subject', '$userdata[uid]', '$forum', '$time'";
	...
	$sql = "INSERT INTO posts (topic_id, image, forum_id, poster_id, post_text, post_time, poster_ip) VALUES ('$topic_id', '$image_subject', '$forum', '$userdata[uid]', '$message', '$time', '$poster_ip')";
	if(!$result = mysql_query($sql, $db)) {
	    forumerror(0020);
	}
	if($userdata[uid] != 1) {
	...
	$topic = $topic_id;
	OpenTable();
	echo "<center>".translate("Your topic has been posted")."<br><a href=\"viewtopic.php?topic=$topic_id&forum=$forum\">".translate("Click here to view your post.")."</center></a>";
	CloseTable();
	}
...
?>
	...
	&nbsp;<a href="viewforum.php?forum=<?php echo $forum?>"><?php echo $forum_name?></a><br>
	<a href=\"forum.php\">Go to <?php echo "$sitename"; ?> Forums Index</a>
	</font></p>
	<?php CloseTable(); ?>
	<FORM ACTION="newtopic.php" METHOD="POST" NAME="coolsus">
...
?>
	...
	<TR ALIGN="LEFT">
		...
		<?php
			if($allow_bbcode == 1) {
		?>	
				<INPUT TYPE="CHECKBOX" NAME="bbcode"><?php echo "".translate("Disable")." <a href=\"bbcode_ref.php\" target=\"_blank\"><i>".translate("BBCode")."</i></a> ".translate("on this Post");?><BR>
		<?php
			}
		?>

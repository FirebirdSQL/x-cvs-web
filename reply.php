...
if($submit) {
	...
	$time = date("Y-m-d H:i");
	$sql = "INSERT INTO posts (topic_id, image, forum_id, poster_id, post_text, post_time, poster_ip) VALUES ('$topic', '$image_subject', '$forum', '$userdata[uid]', '$message', '$time', '$poster_ip')";
	if(!$result = mysql_query($sql, $db)) {
	    forumerror(0020);
	}
	$sql = "UPDATE forumtopics SET topic_time = '$time' WHERE topic_id = '$topic'";
	if(!$result = mysql_query($sql, $db)) {
	    forumerror(0020);
	}
	if($userdata["uid"] != -1) {
	...
	$sql = "SELECT t.topic_notify, u.email, u.uname FROM forumtopics t, users u WHERE t.topic_id = '$topic' AND t.topic_poster = u.uid";
	if(!$result = mysql_query($sql, $db)) {
	    forumerror(0022);
        }
	...
	if($m[topic_notify] == 1 && $m[uname] != $username) {
		...
		$message .= "$nuke_url/viewtopic.php?topic=$topic&forum=$forum\r\nOr view $sitename forum index at $nuke_url/forum.php";
		$message .= "\r\n\r\nThank you for using $sitename forums. \r\nHave a nice day.";
		if (!$system) {
		    mail($m[email], $subject, $message, "From: phpNuke_forum@$SERVER_NAME\r\nX-Mailer: phpNuke_forum");
		}
	}
	OpenTable2();
	echo "<center>".translate("Reply Posted.")."<br><a href=\"viewtopic.php?topic=$topic&forum=$forum\">".translate("Click here to view your topic")."</center><P></font>";
	CloseTable();
	} else {	
	    OpenTable2();
...
list($topic_title) = mysql_fetch_array(mysql_query("select topic_title from forumtopics where topic_id='$topic'"));
OpenTable();
?>
	<P ALIGN=LEFT><FONT SIZE="2"><b><?php echo "".translate("Moderated By: ")."</b><a href=user.php?op=userinfo&uname=$moderator>$moderator";?></a><br>
	<b><?php echo translate("Forum");?>:</b> &nbsp;<a href="viewforum.php?forum=<?php echo $forum?>"><?php echo $forum_name?></a><br>
	<b><?php echo translate("Post Reply in Topic:");?></b>&nbsp;<a href="viewtopic.php?forum=<?php echo $forum?>&topic=<?php echo $topic?>"><?php echo $topic_title?></a><br>
	<a href="forum.php">Go to <?php echo "$sitename"; ?> Forums Index</a>
	</font></p>
<?php CloseTable(); ?>
...
<?PHP
	if (isset($user) && $forum_access == 1) {
	        $user = base64_decode($user);
	        $userdata = explode(":", $user);
	        $sql = "SELECT u.pass, s.level FROM users u, users_status s WHERE u.uid = '$userdata[0]' and s.uid = '$userdata[0]'";
	        $result = mysql_query($sql, $db);
...
<?php
$sql = "SELECT p.*, u.*, s.* FROM posts p, users u, users_status s WHERE topic_id = '$topic' AND p.poster_id = u.uid AND p.poster_id = s.uid";
if(!$result = mysql_query($sql, $db))
    forumerror(0001);
$myrow = mysql_fetch_array($result);
$count=0;
...
if (($count%2)!=0) $color=$bgcolor3;
   ...
   if($myrow[uid] != -1) {
	   if($myrow[rank] != 0) 
	     $sql = "SELECT rank_title FROM ranks WHERE rank_id = '$myrow[rank]'";
	   else
	     $sql = "SELECT rank_title FROM ranks WHERE rank_min <= " . $myrow[posts] . " AND rank_max >= " . $myrow[posts] . " AND rank_special = 0";

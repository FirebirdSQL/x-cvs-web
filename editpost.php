...
if($submit) {
   include("header.php");
   $sql = "SELECT poster_id FROM posts WHERE (post_id = $post_id)";
   ...
   $message = addslashes($message);
	if(!$delete) {
		...
		if(!$result = mysql_query($sql, $db))
		    forumerror(0001);
		OpenTable();
		echo "<center>".translate("Your post has been updated.")."<br><a href=\"viewtopic.php?topic=$topic_id&forum=$forum_id\">".translate("Click here to view the update")."<br><a href=\"viewforum.php?forum=$forum_id\">".translate("Click here to return to the forum topic listing.")."</center><P></font>";
		CloseTable();
	}
	else {
		...
		echo "<center>".translate("Your post has been deleted.")."<br><a href=\"viewforum.php?forum=$forum_id\">".translate("Click here to return to the forum topic listing.")."<br><a href=\"forum.php\">".translate("Click here to return to the forum index.")."</center><P></font>";
	}

} else {

   include("header.php");
   $sql = "SELECT p.*, u.uname, u.uid, u.user_sig FROM posts p, users u WHERE (p.post_id = '$post_id') AND (p.poster_id = u.uid)";  
   if (!$result = mysql_query($sql, $db)) {
	forumerror(0001);
    }
